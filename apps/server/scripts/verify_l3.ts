...
const checks: Array<[string, boolean]> = [
  ...
  [
    'L5 每项卡点成因非空',
    l5.karmaPatterns.every((p) => p.cause.length > 0 && p.manifestation.length > 0),
  ],
  ...
  [
    'L6 分叉点3个且年份递增',
    l6.branchPoints.length === 3 && l6.branchPoints[0].year < l6.branchPoints[1].year,
  ],
  [
    'L6 分叉点年份2035/2045',
    l6.branchPoints[0]?.year === 2035 && l6.branchPoints[1]?.year === 2045,
  ],
  ['L6 分叉点年龄34/44', l6.branchPoints[0]?.age === 34 && l6.branchPoints[1]?.age === 44],
  [
    'L6 分叉点含行运背景',
    l6.branchPoints[0]?.context.includes('乙卯') && l6.branchPoints[1]?.context.includes('丙辰'),
  ],
  ...
  [
    'L7 裁定以日主为纲纳音为参',
    l7.conflictResolution[0]?.ruling.includes('日主') &&
      l7.conflictResolution[0]?.ruling.includes('纳音'),
  ],
  ...
  [
    'L8 七级顺序由外到内',
    l8.levels.map((lv) => lv.name).join('>') ===
      '环境布局>行为抉择>认知思维>习惯体系>因果化解>信息维度重构>心性破执',
  ],
  ...
  [
    'L6 深度模式附行运窗口',
    l6Quantum.depthWindows?.length === 4 && l6Ultimate.depthWindows?.length === 4,
  ],
