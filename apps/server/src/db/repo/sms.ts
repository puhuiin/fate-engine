...
export function createSmsRepo(db: Db) {
  return {
    deleteExpiredByPhone(phone: string): void {
      prepareStmt(db, "DELETE FROM sms_code WHERE phone = ? AND expires_at <= datetime('now')").run(
        phone,
      );
    },
    /** 清理：删除全部已过期验证码，返回删除条数 */
    deleteExpired(): number {
      const info = prepareStmt(db, "DELETE FROM sms_code WHERE expires_at <= datetime('now')").run();
    ...
    invalidateAllUnused(phone: string): void {
      prepareStmt(db, 'UPDATE sms_code SET used = 1 WHERE phone = ? AND used = 0').run(phone);
    },
    insert(phone: string, code: string, expiresAt: string, channel: string): void {
      prepareStmt(db, 'INSERT INTO sms_code (phone, code, expires_at, channel) VALUES (?, ?, ?, ?)').run(
        phone,
        code,
        expiresAt,
        channel,
    ...
    incrementFailCount(id: number, tried: number): void {
      prepareStmt(db, 'UPDATE sms_code SET fail_count = ? WHERE id = ?').run(tried, id);
    },
    ...
    markUsed(id: number): void {
      prepareStmt(db, 'UPDATE sms_code SET used = 1 WHERE id = ?').run(id);
    },
  };
}
