...
export default function History() {
  ...
  const load = async () => {
    const [rec, arc, user] = await Promise.allSettled([listRecords(), listArchives(), getMe()]);
    if (rec.status === 'fulfilled') setRecords((rec.value.data ?? []) as unknown as RecordRow[]);
        ...
        {me?.phone_masked ? (
          ...
          <div className="login-form">
            ...
            <div className="login-row">
              ...
              />
              <button className="ghost" disabled={busy} onClick={sendCode}>
                发送验证码
              </button>
            </div>
            <div className="login-row">
