...
| L2 | 术数八流派 | 八字（含流派统一口径：23 点换日、分秒级起运）、纳音五行、神煞格局、五运六气、十神六亲、调候用神、病药论、刑冲合害并行计算并输出流派标注；八字 V2 深度维度：格局法（月令定格 + 透干成格）、用神喜忌（扶抑 + 季节调候）、神煞（天乙/桃花/驿马/华盖等查表）、地支刑冲合害、十二长生（阳顺阴逆）、藏干透干 |
| L3 | 科学祛魅 | 概率参照系、样本偏差说明，弱化确定性宣称 |
| L4 | 综合研判 | 30% 先天 + 20% 流年 + 50% 人为权重合成综合指数；附用神五行六维补位提示 |
| L5 | 因果溯源 | 识别"求认可与自我证明"类核心卡点（含缺五行模式） |
| L6 | 量子多线 | 四条平行发展线 + 契合度 + 未来分叉点 + 风险提示；支持 standard / quantum / ultimate 三档深度（分叉点 3/5/全部，深度档附各线行运窗口 + 用神与大运生克研判） |
...
| 校验 | 6 组确定性回归脚本（45 + 10 + 43 + 50 + 172 + 11 = 331 断言，verify_all 为聚合入口，各脚本运行均输出断言总数可自动核对）；前端 18 文件 143 用例（含 React 组件/hook 测试） |
...
```bash
# 全量回归（verify_all 45 + 5 组 286 = 331 断言）
npm run verify -w @fate/server

# 分块验证
npm run verify:l1 -w @fate/server   # 真太阳时/跨日/夏令时/排盘子时边界（10 用例）
npm run verify:l2 -w @fate/server   # 八流派算力池 + 深度维度（格局/用神/神煞/刑冲/十二长生/藏干透干）+ 大运顺逆 + 历法边界（43 断言）
npm run verify:l3 -w @fate/server   # L5–L9 确定性输出（50 断言，含深度模式差异/用神研判）
...
│   │       │   ├── l2/       # 术数八流派（八字/纳音/神煞/五运六气/六亲/调候用神/病药论/刑冲合害）
